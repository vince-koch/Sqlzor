...
namespace Sqlzor.Drivers.Services
{
    ...
    {
        public async Task<Dictionary<string, DataTable>> FetchSchemaTables(IDatabaseDriver databaseDriver, string connectionString, int maxConnections)
        {
        ...
        private async Task<DataTable> GetCollection(IDatabaseDriver databaseDriver, string connectionString, string collectionName)
        {
            try
            {
